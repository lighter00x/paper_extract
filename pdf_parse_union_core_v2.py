...
from magic_pdf.pre_proc.cut_image import ocr_cut_image_and_table, ocr_cut_image_and_table_self
...
def process_groups(groups, body_key, caption_key, footnote_key):
    """
    处理分组数据，将body、caption和footnote分离，并在处理图像块时将对应的caption关联上
    """
    body_blocks = []
    caption_blocks = []
    footnote_blocks = []
    ...
    for i, group in enumerate(groups):
        ...
        group[body_key]['group_id'] = i
        # print(group)
        # 如果该分组中存在caption，可以将第一个caption关联到图像块上（或根据需要选择合适的规则）
        # if group.get(caption_key) and len(group[caption_key]) > 0:
        #     # 假设每个caption块中有'text'字段存储实际文本
        #     group[body_key]['caption'] = group[caption_key][0].get('text', '')
...
def parse_page_core(
    ...
    # print("img:", img_body_blocks)
    # print("caption:", img_caption_blocks)
    # print(img_footnote_blocks)
    table_body_blocks, table_caption_blocks, table_footnote_blocks = process_groups(
        table_groups, 'table_body', 'table_caption_list', 'table_footnote_list'
    )
    # print("tab:", table_body_blocks)
    # print("caption:", table_caption_blocks)
    ...
    # print('boxes', all_bboxes)
    # boxes是有区分caption的，但是spans都是text
    """获取所有的spans信息"""
    ...
    # print('before_images', images)


    # """对image和table截图"""
    # print('images:', images)
    if len(images) > 0:
        images = ocr_cut_image_and_table_self(
            images, page_doc, page_id, pdf_bytes_md5, imageWriter
        )
    # print('tables:', tables)
    if len(tables) > 0:
        tables = ocr_cut_image_and_table_self(
            tables, page_doc, page_id, pdf_bytes_md5, imageWriter
        )
    # print('after_images', images)
    # # print('table', tables)'
...
def pdf_parse_union(
    ...
    # print('###pdf_bytes_md5:',pdf_bytes_md5 )
    """初始化空的pdf_info_dict"""
    pdf_info_dict = {}
